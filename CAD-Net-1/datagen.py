...
class ListDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        boxes = self.boxes[idx].clone() # The clone operation corresponds to making a copy of the Tensor contained in this variable.
        #print(boxes)
        #print("RAnjeet")
    ...
    def random_flip(self, img, boxes):
        ...
        '''
        if random.random() < 0.5: # horizontally flipped with probability of 0.5
            img = img.transpose(Image.FLIP_LEFT_RIGHT) # gives us the mirror image of an image 
            w = img.width
            xmin = w - boxes[:,2]
            xmax = w - boxes[:,0]
            boxes[:,0] = xmin
    ...
    def random_crop(self, img, boxes, labels):
        ...
        while True:
            ...
            for _ in range(100):
                ...
                center = (boxes[:,:2] + boxes[:,2:]) / 2  # [N,2] # Getting center of box
                #print(center)
                ...
                if not mask.any():
                    continue
                #print(boxes)
                selected_boxes = boxes.index_select(0, mask.nonzero().squeeze(1)) # squeeze(input, 1) will squeeze the tensor to the shape (A×B), second dimension will be changed.
                #print("Ranjeet")
