...
trainset = ListDataset(root='/media/biometric/Data1/Ranjeet/NewPytorch/SSD_Ear_RGB_300/Cropped_2000_wild', list_file='./voc_data/Cropped_wild_train.txt', train=True, transform=transform)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=20, shuffle=True, num_workers=3)

testset = ListDataset(root='/media/biometric/Data1/Ranjeet/NewPytorch/SSD_Ear_RGB_300/Cropped_2000_wild', list_file='./voc_data/Cropped_wild_test.txt', train=False, transform=transform)
testloader = torch.utils.data.DataLoader(testset, batch_size=20, shuffle=False, num_workers=3)
# Modl
net = SSD300()
if args.resume:
    print('==> Resuming from checkpoint..')
    checkpoint = torch.load('./checkpoint/Updated_ubear_400_and_Wild_ear_2000_images_model.pth')
...
def test(epoch):
    ...
    if test_loss < best_loss:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/Updated_ubear_400_and_Wild_ear_2000_images_model_ssd2.pth')
        best_loss = test_loss
