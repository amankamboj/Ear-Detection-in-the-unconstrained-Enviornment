...
# Load model
net = SSD300()
checkpoint = torch.load('Model/Model_Org.pth')
...
iou_path_Ear = open('Obj_Score_UBEAR.txt', 'w+')
...
for image in list_image:
	...
	#count = count + 1
	try:
	    image_path = '/media/biometric/Data1/Database/Ear_DataSet/UBEAR/Data2/All_tiff/' + image[0]
	    ...
	    for box in boxes:
	        ...
	        IOU = round(abs(iou.bb_intersection_over_union(ground_truth_box, predict_box)), 2)
	        if(IOU < 0.7 and scores1 > 0.8):
	        	draw.rectangle(ground_truth_box, outline='red')
	        	draw.rectangle(predict_box, outline='blue')
	        	draw.text((100,300),str('IOU:'+str(IOU)),font = fnt, fill=(255,0,0))
		        draw.text((100,400),str('Objectness Score:'+str(scores1)),font = fnt, fill=(0,255,0))
	        	img.save('/media/biometric/Data1/Ranjeet/NewPytorch/SSD_Ear_RGB_300/ObjectScore1/' + image[0])
