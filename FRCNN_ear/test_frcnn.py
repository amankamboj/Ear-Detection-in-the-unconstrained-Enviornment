...
for idx, img_name in enumerate(sorted(os.listdir(img_path))):
	...
	for key in bboxes:
		...
		if(max_prob>prob):
			...
			cv2.rectangle(img,(real_x1, real_y1), (real_x2, real_y2), (int(class_to_color[key][1]), int(class_to_color[key][0]), int(class_to_color[key][0])),5)
			print(img_name, real_x1, real_y1, real_x2, real_y2)
	#print('Elapsed time = {}'.format(time.time() - st))
	#print(all_dets)
	
	"""cv2.imshow('img', img)/media/biometric/Data1/Ranjeet/NewPytorch/keras_frcnn/test/
	cv2.waitKey(0)"""
	cv2.imwrite('Challenge_Results/' + img_name ,img)
