...
visualise = True
iou_path = open('UBEAR_IOU_fail.txt', 'wb+')
visualise = True
#path_Rect_txt = ''
fo = open('GT_UBEAR_New_data_updated.txt' , "r")
count=0
...
for line in fo:
	...
	img_name = (filename.strip().split('/'))[9]
	st = time.time()
	filepath = filename
	...
	for key in bboxes:
		...
		cv2.rectangle(img,(x1_org,y1_org,), (x2_org, y2_org),(0, 0, 255),4)
		cv2.rectangle(img,(x1_p, y1_p), (x2_p, y2_p), (255, 0, 0),4)
		print(ground_truth_box)
		print(predict_box )
		#cv2.imwrite('/media/biometric/Data1/Ranjeet/NewPytorch/keras_frcnn/test/' + img_name ,img)
	...
	#print "IOU = " + str(IOU)
		...
		print IOU
		if IOU<=0.1:
			cv2.imwrite('/media/biometric/Data1/Ranjeet/NewPytorch/keras_frcnn/ioutest/' + img_name ,img)
 			cv2.imwrite('/media/biometric/Data1/Ranjeet/NewPytorch/keras_frcnn/miscases/' + img_name ,img1)
